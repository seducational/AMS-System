...
const AppNavbar = () => {
  const { isLoggedIn, userRole, activeTab, setActiveTab, handleLogout } = useAuth();
                ...
                {cotTabs.map(tab => (
                  <Nav.Link
                    ...
                    className={`fw-semibold ${
                      activeTab === tab ? 'text-primary border-bottom border-3 border-primary' : 'text-secondary'
                    }`}
            ...
            {!isLoggedIn && (
              <Nav className="d-flex w-75 justify-content-center">
                <NavLink
                  to="/"
                  className={({ isActive }) =>
                    isActive ? 'pe-4 align-self-center fs-5 custom-active' : 'pe-4 text-dark fs-5 align-self-center'
                  }
          ...
          {isLoggedIn && userRole === 'user' ? (
            ...
            <div className="d-flex align-items-center gap-2">
              <Button className="rounded-4" variant="outline-primary" onClick={() => navigate('/login')}>
                <IoLogInOutline className='me-2 fs-4' />Log In
              </Button>
              <NavLink to="/Register" className="btn btn-primary rounded-4" role="button" onClick={() => navigate('/register')}>
                <IoLogInOutline className='me-2 fs-4' />Register
              </NavLink>
            </div>
          )}
        </Container>
